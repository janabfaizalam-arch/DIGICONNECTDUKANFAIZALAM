...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (image instanceof File && image.size > 0) {
    const upload = await uploadItrImage(image, "desktop");
  ...
  if (mobileImage instanceof File && mobileImage.size > 0) {
    const upload = await uploadItrImage(mobileImage, "mobile");
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const { error } = await supabase.from("itr_section_banners").delete().eq("id", id);
  if (error) return jsonError(error.message, 500);
