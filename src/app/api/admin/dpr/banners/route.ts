...
export async function POST(request: Request) {
  ...
  const upload = await uploadDprImage(image as File, "desktop");
  ...
  if (mobileImage instanceof File && mobileImage.size > 0) {
    const mobileUpload = await uploadDprImage(mobileImage, "mobile");
  ...
  const { data, error } = await supabase.from("dpr_section_banners").insert(payload).select("*").single();
