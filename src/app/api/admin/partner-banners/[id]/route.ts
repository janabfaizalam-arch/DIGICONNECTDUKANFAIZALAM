...
function validateImageFile(file: FormDataEntryValue | null, label: string) {
  ...
  if (file.size > maxImageSize) return `${label} image must be smaller than 8MB.`;
  return null;
}

async function uploadBannerImage(file: File, folder: "desktop" | "mobile") {
  const supabase = getSupabaseAdmin();
  if (!supabase) return { error: "Banner upload is not configured.", path: null, url: null };

  const signature = await validateFileSignature(file, allowedImageTypes);
  if (!signature.valid) return { error: signature.error || "Invalid image file.", path: null, url: null };
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const partnerTypes = parsePartnerTypesFromForm(
    multi.length ? multi.join(",") : nullableText(formData, "partner_types"),
  );
  ...
  if (image instanceof File && image.size > 0) {
    const upload = await uploadBannerImage(image, "desktop");
  ...
  if (mobileImage instanceof File && mobileImage.size > 0) {
    const upload = await uploadBannerImage(mobileImage, "mobile");
