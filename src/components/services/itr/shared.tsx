...
export function SectionShell({
  ...
  const motionProps = useItrMotion();
  return (
    <section
...
function BannerBlock({ banner, lazy }: { banner: ItrBanner; lazy: boolean }) {
  const href = banner.buttonUrl || undefined;
  const content = (
    <div className="relative w-full overflow-hidden rounded-[24px] border border-slate-100 shadow-[0_12px_40px_rgba(15,23,42,0.06)] aspect-[7/3]">
      <Image
        src={banner.mobileImageUrl || banner.imageUrl}
        alt={banner.altText || banner.title || "ITR filing service banner"}
        fill
...
export function DisclaimerText({ text }: { text: string | null }) {
  if (!text) return null;
  return (
    <p className="text-xs text-slate-400 text-center max-w-3xl mx-auto leading-relaxed mt-6">
      {text}
    </p>
  );
}
