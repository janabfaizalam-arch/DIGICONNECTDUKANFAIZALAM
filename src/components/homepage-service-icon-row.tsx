        ...
        <div className="grid grid-cols-4 gap-2 md:grid-cols-8 md:gap-3">
          {services.map(({ label, href, icon: Icon }) => (
            <Link
              key={label}
              href={href}
              className="group flex min-h-[4.35rem] min-w-0 flex-col items-center justify-center gap-1.5 rounded-2xl border border-blue-100 bg-white px-1.5 py-2 text-center shadow-[0_6px_16px_rgba(15,23,42,0.04)] transition-all duration-200 md:min-h-[5rem] md:px-3 md:py-3 md:hover:-translate-y-0.5 md:hover:border-orange-100 md:hover:bg-orange-50/35"
            >
              <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-blue-50 text-blue-700 transition-all duration-200 group-hover:bg-orange-50 group-hover:text-orange-600 md:h-10 md:w-10">
