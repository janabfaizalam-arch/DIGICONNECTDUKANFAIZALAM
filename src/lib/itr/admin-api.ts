...
import { ITR_ADMIN_PATH, ITR_LANDING_PATH, ITR_MEDIA_BUCKET, ITR_SEO_LANDING_PATH } from "./constants";
...
export function revalidateItrPaths() {
  revalidatePath(ITR_LANDING_PATH);
  revalidatePath(ITR_SEO_LANDING_PATH);
  revalidatePath(ITR_ADMIN_PATH);
}
...
export async function uploadItrImage(file: File, folder: "desktop" | "mobile") {
  ...
  const { error } = await supabase.storage.from(ITR_MEDIA_BUCKET).upload(storagePath, file, {
  ...
  const { data } = supabase.storage.from(ITR_MEDIA_BUCKET).getPublicUrl(storagePath);
